...
const AnnonceDetailsUser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const voitureData = await fetch(`${getUrl()}/api/accueil/selectByIdVoiture?id_voiture=${id}`, {method: 'GET' });
        ...
        <div id='box1'>
          ...
          {voiture.map((caracteristique, index) => (
            <React.Fragment key={index}>
              ...
              <label><span className='labelTitle'>Longueur:</span> {caracteristique.longueur} </label>
